...
const DetailsInput = () => {
  ...
  const { details, setDetails, errorFields, setRemovedDetail, removedDetail } =
    useProductContext();
  ...
  const handleRemoveColumnDetails = (indexDetail, indexItem) => {
    ...
    if (updatedDetails[indexDetail]?.id) {
      updatedDetails[indexDetail].isUpdated = true;
    }
    ...
    if (updatedDetails[indexDetail].items.length === 1) {
      if (updatedDetails[indexDetail]?.id) {
        setRemovedDetail([
          ...removedDetail,
          { id: updatedDetails[indexDetail]?.id },
        ]);
      }
